...
def aggregate_parquet(df_parquet):
    ...
    # Keep your extra threshold + trend features
...
def save_predictions_csv(filename, X, y_true, clf_pred, reg_pred, combined_pred, id_series=None):
    ...
    # Include mapping identifier if provided
    """
    if id_series is not None:
        try:
            ser = pd.Series(id_series).reset_index(drop=True).astype(str).str.upper()
            df_out.insert(0, 'wb_id', ser)
        except Exception:
            # Best effort
            df_out['wb_id'] = pd.Series(id_series).astype(str).str.upper()
...
def main(args):
    ...
    # --- keep waterbody IDs aligned for export (do not feed into model) ---
    wb_ids_all = None
    if 'wb_id' in df_model.columns:
        wb_ids_all = df_model['wb_id'].astype(str).str.upper().copy()
# Drop unique ID column to reduce dimensionality and avoid overfitting
    ...
    if wb_ids_all is not None:
        wb_ids_all = wb_ids_all[mask].reset_index(drop=True)
    ...
    # Align IDs to the split (indices are preserved for DataFrames)
    wb_id_train = None
    wb_id_test = None
    if wb_ids_all is not None and isinstance(X, pd.DataFrame):
        try:
            wb_id_train = wb_ids_all.loc[X_train.index]
            wb_id_test  = wb_ids_all.loc[X_test.index]
        except Exception:
            # Fallback: positional alignment
            wb_id_train = wb_ids_all.iloc[:len(X_train)]
            wb_id_test  = wb_ids_all.iloc[:len(X_test)]
    ...
    # --- Filter low-variance features to help LightGBM ---
    ...
    if target_type == "categorical":
        lgbm = LGBMClassifier(
            n_estimators=300,        # fewer trees to start
            learning_rate=0.05,
            num_leaves=31,
            class_weight='balanced',
            random_state=RANDOM_STATE
...
def explain_model(model, X, feature_names=None, target_name="target", top_n=15):
    ...
    # ---------- 1D PDPs (internal use only; you already save 1D via save_1d_pdp_plots) ----------
    # If you still want these computed here for text summaries, make sure we pass arrays:
    for idx, fname in zip(top_idx, top_features):
        ...
        # You can keep whatever summary logic you had here if needed...
        # (omitted for brevity)
            ...
            if f1 in feature_names and f2 in feature_names:
                ...
                try:
                    ...
                    _ = PartialDependenceDisplay.from_estimator(
                        model,
                        X_pdp.values,                 # <<< array, not DataFrame
                        [(idx1, idx2)],
...
def save_1d_pdp_plots(
    ...
    # choose features to plot (by importance if available)
    if hasattr(model, "feature_importances_"):
    ...
    # which class targets to plot (for classifiers)
    class_targets = None
    class_labels = None
    ...
    for idx in feat_indices:
        ...
        if is_classifier and class_targets is not None and len(class_targets) > 0:
            ...
            for targ in class_targets:
                ...
                try:
                    ...
                    PartialDependenceDisplay.from_estimator(
                        ...
                        kind="average",  # robust choice across versions
                        target=targ, ax=ax,
                    )
                except TypeError:
                    # fallback for older sklearn (no kind/target args)
                    try:
                        PartialDependenceDisplay.from_estimator(
                            model, X_df, features=[idx],
            ...
            # regression (or classification w/o explicit targets)
            fig, ax = _fresh_ax()
            try:
                PartialDependenceDisplay.from_estimator(
                    model, X_df, features=[idx],
